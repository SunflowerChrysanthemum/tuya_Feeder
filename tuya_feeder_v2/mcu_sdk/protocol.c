...
{
  {DPID_TOTAL_WEIGHT, DP_TYPE_VALUE},
  {DPID_TUB_WEIGHT, DP_TYPE_VALUE},
  {DPID_SWITCH_AUTO, DP_TYPE_BOOL},
  {DPID_SMALL_FEED, DP_TYPE_BOOL},
  {DPID_MIDDLE_FEED, DP_TYPE_BOOL},
  {DPID_BIG_FEED, DP_TYPE_BOOL},
};
...
{
    mcu_dp_value_update(DPID_TOTAL_WEIGHT, total_weight); //VALUE型数据上报;
    mcu_dp_value_update(DPID_TUB_WEIGHT, tub_weight); //VALUE型数据上报;
//    mcu_dp_bool_update(DPID_SWITCH_AUTO,auto_feed); //BOOL型数据上报;
}
...
******************************************************************************/
/*****************************************************************************
函数名称 : dp_download_switch_auto_handle
功能描述 : 针对DPID_SWITCH_AUTO的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_switch_auto_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:off/1:on
    unsigned char switch_auto;
    
    switch_auto = mcu_get_dp_download_bool(value,length);
    if(switch_auto == 0) {
        //bool off
		flag_auto = 1;
    }else {
    ...
    ret = mcu_dp_bool_update(DPID_SWITCH_AUTO,switch_auto);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_small_feed_handle
功能描述 : 针对DPID_SMALL_FEED的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 只下发类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_small_feed_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:off/1:on
    unsigned char small_feed;
    
    small_feed = mcu_get_dp_download_bool(value,length);
    if(small_feed == 0) {
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_bool_update(DPID_SMALL_FEED,small_feed);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_middle_feed_handle
功能描述 : 针对DPID_MIDDLE_FEED的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 只下发类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_middle_feed_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:off/1:on
    unsigned char middle_feed;
    
    middle_feed = mcu_get_dp_download_bool(value,length);
    if(middle_feed == 0) {
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_bool_update(DPID_MIDDLE_FEED,middle_feed);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_big_feed_handle
功能描述 : 针对DPID_BIG_FEED的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 只下发类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_big_feed_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:off/1:on
    unsigned char big_feed;
    
    big_feed = mcu_get_dp_download_bool(value,length);
    if(big_feed == 0) {
        //bool off
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_bool_update(DPID_BIG_FEED,big_feed);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    unsigned char ret;
    switch(dpid) {
        case DPID_SWITCH_AUTO:
            //自动喂食处理函数
            ret = dp_download_switch_auto_handle(value,length);
        break;
        case DPID_SMALL_FEED:
            //小份喂食处理函数
            ret = dp_download_small_feed_handle(value,length);
        break;
        case DPID_MIDDLE_FEED:
            //中份喂食处理函数
            ret = dp_download_middle_feed_handle(value,length);
        break;
        case DPID_BIG_FEED:
            //大份喂食处理函数
            ret = dp_download_big_feed_handle(value,length);
        break;
